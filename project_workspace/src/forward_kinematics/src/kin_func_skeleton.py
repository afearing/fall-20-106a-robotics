...
def rotation_3d(omega, theta):
    ...
    sine = (1/omega_norm) * omega_hat *np.sin(theta * omega_norm)
    cosi = (1 / (omega_norm ** 2)) * (np.matmul(omega_hat, omega_hat)) * (1 - np.cos(omega_norm * theta))
    rotMat = identity + sine + cosi
    return rotMat
...
def homog_3d(xi, theta):
    ...
    if np.allclose(w, 0):
        ...
        rotationMat = rotation_3d(w, theta)
        omegaNormSquared = np.linalg.norm(w) ** 2 
        # subpart1 = np.matmul((np.eye(3) - rotationMat), np.matmul(skew_3d(w), v))
        subpart1 = np.matmul((np.eye(3) - rotationMat), np.dot(skew_3d(w), v))
        subpart2 = np.dot(np.outer(w, w.T), theta * v)
        R = rotationMat
        p = (1/omegaNormSquared) * (subpart1 + subpart2)
    g = np.eye(4)
    g[:3, :3] = R
    g[:3, 3] = p
    return g
...
def prod_exp(xi, theta):
    ...
    for i in range(len(theta)):
    	result = np.matmul(result, homog_3d(xi[:,i],theta[i]))
    return result
...
def twist(omega, q):
    ...
    if not q.shape == (3,):
        raise TypeError('xi must be a 3-vector')
    return np.array([[np.cross(-omega, q)], [omega]]).flatten()
