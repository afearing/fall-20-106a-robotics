...
def rotation_3d(omega, theta):
    ...
    w_hat = skew_3d(omega)
    return np.eye(3) + (w_hat/w_mag)*np.sin(w_mag*theta) + np.linalg.matrix_power(w_hat/w_mag, 2)*(1 - np.cos(w_mag*theta))
...
def homog_3d(xi, theta):
    ...
    if np.allclose(w, 0):
        ...
        # Translation and rotationNone
        R = rotation_3d(w, theta)
        p_term1 = np.matmul(np.eye(3) - R, np.matmul(skew_3d(w), v))
        p_term2 = np.matmul(np.outer(w, w), theta*v)
        p = (p_term1 + p_term2)/np.linalg.norm(w)**2
    g = np.eye(4)
    g[:3, :3] = R
    g[:3, 3] = p
    return g
...
def prod_exp(xi, theta):
    ...
    for k in range(len(theta)):
        g = np.matmul(g, homog_3d(xi[:, k], theta[k]))
    return g
#-----------------------------Testing code--------------------------------------
